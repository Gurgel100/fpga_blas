...
void ReadMemory(Data_t const memory[], Stream<Data_t> &pipe, const size_t N) {
  for (size_t i = 0; i < N; ++i) {
    #pragma HLS PIPELINE II=1
    pipe.Push(memory[i]);
  }
...
void WriteMemory(Stream<Data_t> &pipe, Data_t memory[], const size_t N) {
  for (size_t i = 0; i < N; ++i) {
    #pragma HLS PIPELINE II=1
    memory[i] = pipe.Pop();
  }
...
void blas_dot(Data_t const memoryIn_X[], Data_t const memoryIn_Y[], Data_t memoryOut[], const size_t N) {
